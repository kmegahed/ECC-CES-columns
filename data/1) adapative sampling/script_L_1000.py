...
Astr=50;S_space=1000*50*2/min(B,H);fstr=250.0;As=ro_rebar*B*H/4.0;cover=30;
L=lam*B;H2=H/2.0;B2=B/2.0;hs2=(H-2.0*cH)/2.0;bf2=(B-2.0*cB)/2.0;
A_rebar_4=(As*fy+0.85*B*H*0.97*fe)*d_steel/fs/4.0;tw2=A_rebar_4/((bf2-4.0)*tf_tw+hs2);tf=(A_rebar_4-hs2*tw2)/(bf2-tw2)
...
mdb.models[mod1].materials['steel'].Plastic(table=((fy, 0.0), (fy*1.02,0.01)))
...
mdb.models[mod1].materials['rebars'].Plastic(table=((fs, 0.0), (fs*1.02,0.01)))
...
xi.concreteDamagedPlasticity.ConcreteTensionStiffening(table=((fco/10.0, 0.0), (fco/100.0, 0.01)))
...
n2=[]
for i in n:
    n2.append([i.label,i.coordinates[0]+L/1000.0*np.sin(i.coordinates[2]/L*pi),i.coordinates[1],i.coordinates[2]])
...
n2=[]
for i in n:
    n2.append([i.label,i.coordinates[0],i.coordinates[1],i.coordinates[2]-L/1000.0*np.sin(i.coordinates[0]/L*pi)])
...
for i in range(n_Rep):
    ...
    a.translate(instanceList=('Part-4-'+str(i+1), ), vector=(L/1000.0*np.sin(zz/L*pi), 0.0, zz))
...
mdb.models[mod1].DisplacementBC(name='BC-1', createStepName='Step-1',  region=region, u1=0.0, u2=0.0, u3=L/285.0, ur1=UNSET, ur2=UNSET, ur3=0.0, amplitude=UNSET, fixed=OFF, distributionType=UNIFORM, fieldName='',  localCsys=None)
a = mdb.models[mod1].rootAssembly
r1 = a.referencePoints
refPoints2=(r1[RFid2], )
